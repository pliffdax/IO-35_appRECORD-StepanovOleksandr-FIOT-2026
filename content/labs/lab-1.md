...
### 1.1 Тема
**Тема:** «Вибір предметної області. Аналіз, моделювання та розроблення адаптивного web-застосунку (Helpdesk / Ticket System)».
...
|---|---|---|---|---|---|
| UC1 | Create Ticket | User | Відкрита сторінка створення | Заповнює форму → підтверджує | Заявка створена (UI / план: БД) |
| UC2 | View Ticket List | User/Agent | Доступна сторінка списку | Переглядає перелік | Список відображено |
| UC3 | View Ticket Details | User/Agent | Існує заявка | Відкриває деталі | Дані заявки відображено |
| UC4 | Add Comment | User/Agent | Відкрита детальна сторінка | Вводить коментар → додає | Коментар відображено (план: БД) |
| UC5 | Change Status | Agent | Є права агента | Обирає новий статус | Статус оновлено (план: історія) |
